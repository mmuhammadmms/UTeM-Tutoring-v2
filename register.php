...
$sql3 = "select count(*) as countt from users where username = :username or email = :email";
	...
	if ($count > 0){
		echo ("<SCRIPT LANGUAGE='JavaScript'>
 window.alert('Username or email has been used.Please choose different username or email.')
 window.location.href='register.php';
 </SCRIPT>");
	
	}else{
			$sql = "INSERT INTO users VALUES (DEFAULT , :name , :email, '' , '' ,'' , '' , 'profile2.png' , :password , :username , 'Member')";
	$stmt = oci_parse($conn, $sql);

	oci_bind_by_name($stmt, ':name' , $_POST["name"]);
	oci_bind_by_name($stmt, ':email', $_POST["email"] );
	oci_bind_by_name($stmt, ':password', $_POST["pass"] );
...
<!-- stats -->
	...
	<div class="limiter">
		<div class="container-login100">
			<div class="login100-more" style="background-image: url('images/utem3.jpg');"></div>
